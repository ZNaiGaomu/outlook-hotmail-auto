...
def show_banner():
    ...
    print('  Headless / protocol edition')
...
def run_concurrent_flows(controller, concurrent_flows=10, max_tasks=100, max_consecutive_failures=3):
    task_counter = 0
    succeeded_tasks = 0
    failed_tasks = 0
    ...
    with ThreadPoolExecutor(max_workers=concurrent_flows) as executor:
        ...
        while task_counter < max_tasks or len(running_futures) > 0:
            ...
            if (not stopped_early
                ...
                print(f"[Abort] 连续失败 {consecutive_failures} 次，停止提交新任务。"
                      f"常见原因：IP 被风控、chromium 未安装、代理不通。")
                stopped_early = True

            should_submit = (not stopped_early) and task_counter < max_tasks
            while should_submit and len(running_futures) < concurrent_flows:
                new_future = executor.submit(process_single_flow, controller)
                running_futures.add(new_future)
                task_counter += 1
                progress_interval = max(max_tasks // 2, 1)
                if task_counter % progress_interval == 0:
